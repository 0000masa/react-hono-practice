...
```
ブラウザ                        CloudFront / Vite proxy            バックエンド (Lambda / Hono)
──────                         ──────────────────────             ─────────────────────────────
signIn.social()
  → POST /api/auth/sign-in/social ─── /api/* を転送 ──→ BetterAuth が認可URL生成
  ← { url: "https://accounts.google.com/..." }
  → window.location.href = url

  Google 認証画面で認証
       ↓
GET {FRONTEND_URL}/api/auth/callback/google
                                  ─── /api/* を転送 ──→ BetterAuth が自動処理
                                                         ├ トークン交換
                                                         ├ ユーザー情報取得
...
getSession()
  → GET /api/auth/get-session ──── /api/* を転送 ──→ BetterAuth がセッション検証
  ← { session, user }
```

**ポイント**: OAuth コールバック URL も含め、すべてのリクエストはフロントエンドと同一ドメインを経由する。
バックエンドに直接アクセスすることはない（本番では CloudFront + WAF を通すため）。
...
削除:  GOOGLE_CALLBACK_URL — BetterAuth が baseURL + basePath から自動決定
...
承認済みリダイレクト URI を変更:
- **旧**: `http://localhost:3000/api/auth/google/callback`（バックエンド直接）
- **新**: `http://localhost:5173/api/auth/callback/google`（フロントエンド経由）

BetterAuth は `{baseURL}{basePath}/callback/{provider}` の形式でコールバック URL を生成する。
`baseURL` が `http://localhost:5173` なので、Google にはフロントエンドの URL が渡される。

本番環境では `https://{フロントエンドドメイン}/api/auth/callback/google` を登録する。

---

## baseURL をフロントエンドにする理由

### インフラ構成

本番環境では CloudFront がフロントエンドとバックエンドを**同一ドメイン**で配信する:

```
ブラウザ → https://app.example.com
              │
              ├─ /api/*  → CloudFront → API Gateway → Lambda (バックエンド)
              │              └ WAF で不正リクエストをブロック
              │
              └─ その他   → CloudFront → S3 (フロントエンド静的ファイル)
```

ローカル開発では Vite の dev server が同じ構成を再現する:

```
ブラウザ → http://localhost:5173
              │
              ├─ /api/*  → Vite proxy → http://hono:3000 (バックエンド)
              │
              └─ その他   → Vite dev server (フロントエンド)
```

### なぜバックエンドの URL ではダメなのか

BetterAuth の `baseURL` は **Google に渡すコールバック URL の組み立て** に使われる:

```
コールバック URL = baseURL + basePath + /callback/google
```

| baseURL の設定 | 生成されるコールバック URL | 結果 |
|---|---|---|
| `http://localhost:3000` (バックエンド) | `http://localhost:3000/api/auth/callback/google` | Google → バックエンド直接 (CloudFront/WAF をバイパス) |
| `http://localhost:5173` (フロントエンド) | `http://localhost:5173/api/auth/callback/google` | Google → フロントエンド → proxy → バックエンド |

バックエンドの URL を指定すると:
1. **本番で CloudFront + WAF をバイパスしてしまう**（セキュリティ上 NG）
2. **API Gateway は直接アクセスを拒否している**ため 403 エラーになる
3. OAuth コールバック後のリダイレクト先（`/dashboard`）がバックエンドのオリジンで解決され、フロントエンドの画面が出ない

フロントエンドの URL を指定すれば、すべてのトラフィックが CloudFront（本番）/ Vite proxy（ローカル）を通り、統一されたルーティングになる。

### callbackURL は相対パスでよい

```typescript
await authClient.signIn.social({
  provider: 'google',
  callbackURL: '/dashboard',  // 相対パス
});
```

OAuth 処理後のリダイレクトはバックエンドが `302 Location: /dashboard` を返す。
ブラウザは**現在のオリジン**（`http://localhost:5173` や `https://app.example.com`）を基準に解決するので、フロントエンドの `/dashboard` に正しく到達する。
...
### Google ログイン後に 404 / 403 になる

1. Google Cloud Console のリダイレクト URI が**フロントエンドの URL** で BetterAuth の形式になっているか確認:
   - **正**: `http://localhost:5173/api/auth/callback/google`（フロントエンド経由）
   - **誤**: `http://localhost:3000/api/auth/callback/google`（バックエンド直接）
   - **誤**: `http://localhost:5173/api/auth/google/callback`（旧 Arctic 形式）

2. BetterAuth の `baseURL` がフロントエンドの URL になっているか確認:
   ```typescript
   baseURL: env.FRONTEND_URL,  // "http://localhost:5173" ← バックエンドのポートではない
   ```

### Google ログイン後にバックエンドの URL にリダイレクトされる

`baseURL` がバックエンドの URL（`http://localhost:3000`）になっている可能性がある。
フロントエンドの URL に変更すること。詳しくは「baseURL をフロントエンドにする理由」セクションを参照。
