...
- `qrcode` のような overload を持つ API では、`vi.mocked(...)` 側で別途キャストが必要 (詳細は [8.2](#82-qrcodetobuffer-の-overload-で-vimocked-の型推論がズレる) を参照)
...
**症状**: `mockedToBuffer.mockResolvedValue(Buffer.from(''))` で `Argument of type 'Buffer' is not assignable to parameter of type 'void'` のエラーが出る。

#### 「オーバーロード (overload)」とは

オーバーロードは **「同じ関数名に対して、TypeScript の型定義上は複数のシグネチャを宣言する」** 仕組み。`qrcode` の型定義 (抜粋) はこうなっている:

```ts
// callback 版 (戻り値 void)
function toBuffer(text: string, cb: (err: Error|null, buf: Buffer) => void): void;
function toBuffer(text: string, options: QRCodeToBufferOptions, cb: (...) => void): void;

// Promise 版 (戻り値 Promise<Buffer>)
function toBuffer(text: string): Promise<Buffer>;
function toBuffer(text: string, options: QRCodeToBufferOptions): Promise<Buffer>;
```

`toBuffer` という同じ名前に対して、呼び方が **4 通り** あり、それぞれ戻り型が違うことを TypeScript に教える宣言。コンパイラは呼び出しコードの引数を見て「これは何番目の signature か」を自動で判定する。

```ts
QRCode.toBuffer('hello', (err, buf) => {...});  // → 1 番目 (戻り値 void)
QRCode.toBuffer('hello');                        // → 3 番目 (戻り値 Promise<Buffer>)
```

#### ランタイムは関数 1 つだけ

ここがポイント。**JavaScript には overload という概念がない**。実体は **常にたった 1 つの関数** で、その中で「引数の形」を見て処理を分岐する。`qrcode` の実装は概ねこういう形:

```js
function toBuffer(text, optionsOrCb, maybeCb) {
  // 第 2 引数が関数だったら「options 省略 + callback あり」と解釈
  let cb = maybeCb;
  ...
  if (cb) {
    // ── callback 版 ──
    doEncode(text, options, cb);
    return;                       // ← void
  } else {
    // ── Promise 版 ──
    return new Promise((resolve, reject) => {  // ← Promise<Buffer>
      doEncode(text, options, (err, buf) => err ? reject(err) : resolve(buf));
    });
  }
}
```

つまり「引数によって実行される処理が変わる」が答え。実装が `typeof arg === 'function'` などで呼び方を読み取って branch する二刀流 API になっている。TypeScript の overload 宣言は、この実装側の分岐を **型レベルで再現** したもの。

歴史的経緯: 古い Node の API は callback 中心 (例: `fs.readFile(path, cb)`)。後から Promise が普及したとき、既存ユーザーを壊さず Promise 版も追加するために二刀流にした、というのが背景。近年は `fs/promises` のように **モジュールごと分ける** スタイルが主流だが、`qrcode` のような古めのライブラリには残っている。

#### テストで何が起きていたか

`vi.mocked(QRCode.toBuffer)` を素で呼ぶと、TypeScript は 4 つの overload のうちどれを「型」として採用するか自動選択する。多くの場合 **最初に宣言された callback 版 (戻り値 `void`)** が選ばれ、その結果 `mockResolvedValue` の引数型が「`Awaited<void>` = `void`」になって `Buffer` が拒否される。

#### 対処: `vi.mocked` に渡す前に overload を 1 本に絞り込む
...
`QRCode.toBuffer as (...) => Promise<Buffer>` で **「これは Promise 版である」** と TypeScript に明示してから `vi.mocked` に渡すことで、以降の `mockResolvedValue(Buffer)` / `mock.calls` の型が自然に通る。`as unknown as ...` のようなダブルキャストは不要。

#### 補足: 「`vi.fn<>()` で型を付ければ済む」と思いがちだが…
...
この `vi.fn<...>()` は runtime の差し替え対象 (= モック関数) の型を決めているだけで、**`vi.mocked(QRCode.toBuffer)` の戻り型推論には影響しない**。`vi.mocked` は `import QRCode from 'qrcode'` した時点の `@types/qrcode` の型を見るため、別の場所で対応が必要になる。
