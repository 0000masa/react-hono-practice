...
```bash
esbuild src/lambda.ts --bundle --platform=node --outfile=dist/lambda.js --target=node22 --external:@aws-sdk/*
```
...
```json
"build:lambda": "esbuild src/lambda.ts src/sqs-handler.ts src/migrate.ts src/daily-report.ts --bundle --platform=node --outdir=dist --target=node22 --external:@aws-sdk/*"
```
