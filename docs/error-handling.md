...
本ドキュメントは `backend/src/` のエラーハンドリング設計と、新規ハンドラ・ライブラリ追加時の書き方ルールを定める。CloudWatch 側のインフラ設計 (subscription filter / SES 通知 Lambda) については [cloudwatch-error-monitoring-guide.md](./cloudwatch-error-monitoring-guide.md) を参照。
...
詳細は [cloudwatch-error-monitoring-guide.md](./cloudwatch-error-monitoring-guide.md) を参照。
